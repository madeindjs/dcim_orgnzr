...
function moveToNotFound(imagePath, reason) {
  let newPath = path.join(path.dirname(imagePath), "errors", reason, path.basename(imagePath));
...
function organizeImage(imagePath, exifData) {
  // convert longitude
  let longitudeDms = exifData.gps.GPSLongitude;
  let longitude = gpsUtil.toDD(longitudeDms[0], longitudeDms[1], longitudeDms[2]);
  // convert latitude
  ...
  if (intervention !== undefined && intervention.reference) {
    ...
    newPath = path.join(path.dirname(imagePath), sanitize(project.name), "not_found", path.basename(imagePath));
  }
